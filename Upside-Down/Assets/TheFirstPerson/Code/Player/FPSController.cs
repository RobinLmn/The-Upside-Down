    ...
    public class FPSController : MonoBehaviour
    {
        ...
        void UpdateMovement()
        {
            ...
            if (grounded && groundAngle >= controller.slopeLimit && slopeSlideEnabled)
            {
                ...
                else if (inverted && Physics.Raycast(transform.position, Vector3.up, out ledgeCheck, 0.1f + (controller.radius * 2)))
                {
                    if (Vector3.Angle(ledgeCheck.normal, Vector3.down) >= controller.slopeLimit)
                    {
                        slide = true;
                    }
                    else
            ...
            else
            {
                ...
                {
                    if (jumpPressed > 0 && !jumping && jumpReady)
                    {
                        Jump();
                    }
                }
        ...
        void MouseLook()
        {
            if (mouseLookEnabled)
            {
                ...
                if (verticalLookEnabled)
                {
                    ...
                    cam.localEulerAngles = new Vector3(verticalLook, 0, cam.localEulerAngles.z);
                }

                transform.localEulerAngles = new Vector3(0, horizontalLook, cam.localEulerAngles.z);
                //print("lookenabled");
            }
            else
            {
